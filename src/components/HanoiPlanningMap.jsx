...
export default function HanoiPlanningMap({
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: {
        ...
        sources: {
          ...
          metro: { type: 'geojson', data: publicAsset('metro-hanoi.geojson') },
          metrop: { type: 'geojson', data: publicAsset('metro-hanoi-planned.geojson') },
          gadk: { type: 'geojson', data: publicAsset('metro-hanoi-ga-dukien.geojson') },
          apt: { type: 'geojson', data: publicAsset('hanoi-airports.geojson') }
        },
        layers: [
          {
            id: 'bg',
    ...
    map.on('error', (event) => {
      // MapLibre emits recoverable tile/network errors as well as fatal style errors.
      // Keep the error visible in DevTools without crashing React.
      if (event?.error) console.error('[HanoiPlanningMap]', event.error);
    });
  ...
  useEffect(() => {
    ...
    applyLayerState();
    map.once('load', applyLayerState);

    return () => {
      map.off('load', applyLayerState);
    };
